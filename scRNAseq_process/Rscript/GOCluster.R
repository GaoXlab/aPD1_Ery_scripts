...
GOChord <- function(data, title, space, gene.order, gene.size, gene.space, nlfc = 1, lfc.col, lfc.min, lfc.max, ribbon.col, border.size, process.label, limit, scale.title){
  ...
  g<- ggplot() +
    geom_polygon(data = df_process, aes(x, y, group=id), fill='gray70', inherit.aes = F,color= 'white', linewidth=0.1) +
    geom_polygon(data = df_process, aes(x, y, group=id), fill= cols, inherit.aes = F, alpha=0.6, color='#F0F0F0', linewidth=0.1) +	
    geom_point(aes(x = xpro, y = ypro, size = factor(labels, levels = labels), shape = NA), data = df_texp) +
    guides(size = guide_legend("", ncol = 1, byrow = TRUE, override.aes = list(shape = 22, fill = unique(cols), size = 8))) +
    ...
    geom_polygon(aes(x = lx, y = ly, group = ID), data = df_path, fill = colRibb, color = 'gray70', size = border.size, inherit.aes = FALSE, linewidth=0.1) +		
    labs(title = title) +
    theme_blank
  
  if (nlfc >= 1){
    g + geom_polygon(data = df_genes, aes(x, y, group = id, fill = logFC), inherit.aes = F, color = '#F0F0F0', size=0.01, linewidth=0.1) +
      ...
      theme(legend.position = 'right', legend.background = element_rect(fill = 'transparent'), legend.box = 'horizontal', legend.direction = 'horizontal')
  }else{
...
# ggplot() +
#     geom_polygon(data = df_process, aes(x, y, group=id), fill='#F0F0F0', inherit.aes = F,color= 'white', linewidth=0.1) +
#     geom_polygon(data = df_process, aes(x, y, group=id), fill= cols, inherit.aes = F, alpha=0.6, color='#F0F0F0', linewidth=0.1) +	
#     geom_point(aes(x = xpro, y = ypro, size = factor(labels, levels = labels), shape = NA), data = df_texp) +
#     guides(size = guide_legend("", ncol = 1, byrow = TRUE, override.aes = list(shape = 22, fill = unique(cols), size = 8))) +
#     theme(legend.text = element_text(size = process.label)) +
#     geom_text(aes(xgen, ygen, label = labels, angle = angle), data = df_texg, size = gene.size) +
#     geom_polygon(aes(x = lx, y = ly, group = ID), data = df_path, fill = colRibb, color = NA, size = border.size, inherit.aes = FALSE, linewidth=0.1) +
#     geom_polygon(data = df_genes, aes(x, y, group = id, fill = logFC), inherit.aes = F, color = NA, size=0.01) +
#     scale_fill_gradient2(scale.title, space = 'Lab', low = lfc.col[3], mid = lfc.col[2], high = lfc.col[1], guide = guide_colorbar(title.position = "top", title.hjust = 0.5), 
#                          breaks = c(min(df_genes$logFC), max(df_genes$logFC)), labels = c(round(min(df_genes$logFC)), round(max(df_genes$logFC)))) +
#     theme(legend.position = 'right', legend.background = element_rect(fill = 'transparent'), legend.box = 'horizontal', legend.direction = 'horizontal')